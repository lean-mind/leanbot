  ...
  describe("should print error in console and save in file", () => {
    ...
    it("onDbError", () => {
      const messageExpected =
        `${getDateFormatted(date)} Oops! There was an error writing or reading database: ` + JSON.stringify(errorToTest)
    ...
    it("onFileReadError", () => {
      ...
      const messageExpected =
        `${getDateFormatted(date)} Oops! There was an error reading ${file} file: ` + JSON.stringify(errorToTest)
    ...
    it("onFileWriteError", () => {
      const file = "irrelevant-file.log"
      const messageExpected =
        `${getDateFormatted(date)} Oops! There was an error writing ${file} file: ` + JSON.stringify(errorToTest)
    ...
    it("onMissingTranslation", () => {
      ...
      const messageExpected = `${getDateFormatted(
        date
      )} Oops! There was an error in ${file} file: { "${type}": { "${key}": "MISSING TRANSLATION" } }`
